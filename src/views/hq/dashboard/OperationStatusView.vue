...
const inventoryRisks = [
  { item: '코튼 베이직 반팔 티셔츠', category: '상의 > 반팔', location: '이천 풀필먼트', status: '부족', stock: 74, safety: 90 },
  { item: '코튼 베이직 반팔 티셔츠', category: '상의 > 반팔', location: '부산 물류창고', status: '품절', stock: 0, safety: 70 },
  { item: '플리츠 롱스커트', category: '치마 > 롱스커트', location: '인천 제1창고', status: '부족', stock: 18, safety: 55 },
  { item: '라이트 숏 패딩', category: '아우터 > 패딩', location: '대전 허브창고', status: '품절', stock: 0, safety: 30 },
  { item: '오버핏 옥스포드 셔츠', category: '상의 > 셔츠', location: '인천 제1창고', status: '안전', stock: 490, safety: 130 },
]
...
const alerts = [
  { type: '매장 발주량 이상 감지', message: '성수 쇼룸 반팔 티셔츠 발주량 평균 대비 3배 초과', time: '8분 전' },
  { type: '창고 발주량 이상 알림', message: '이천 풀필먼트 플리츠 롱스커트 대량 발주 이상 감지', time: '21분 전' },
  { type: '매장 재고 부족 알림', message: '홍대 플래그십 후드티 안전재고 미달 보충 필요', time: '42분 전' },
  { type: '창고 재고 부족 알림', message: '부산 물류창고 라이트 숏 패딩 재고 부족', time: '1시간 전' },
]
...
const moveImbalance = (direction) => {
  currentImbalanceIndex.value =
    (currentImbalanceIndex.value + direction + warehouseImbalances.length) % warehouseImbalances.length
}
    ...
    <div class="flex flex-col gap-4">
      <section class="border border-gray-200 bg-white p-4 shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p class="text-[10px] font-black uppercase tracking-[0.18em] text-gray-400">HQ Operation Dashboard</p>
          ...
          <div class="flex items-center gap-2">
            <span class="inline-flex h-8 items-center gap-1 border border-[#D6EAEA] bg-[#EBF5F5] px-3 text-[11px] font-black text-[#004D3C]">
              <Clock :size="13" />
              {{ dateLabel }}
            </span>
            <span class="inline-flex h-8 items-center gap-1 border border-emerald-200 bg-emerald-50 px-3 text-[11px] font-black text-emerald-700">
      ...
      <section class="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(360px,0.9fr)]">
        <article class="border border-gray-200 bg-white shadow-sm">
          <div class="flex items-center justify-between border-b border-gray-100 px-4 py-3">
            ...
            <button type="button" class="text-xs font-black text-[#004D3C] hover:underline" @click="goTo('/hq/inventory/company-wide')">
              전사 재고 조회
            </button>
          </div>
          <div class="overflow-x-auto">
            <table class="w-full min-w-[680px] text-left text-xs">
              ...
              <tbody class="divide-y divide-gray-100">
                <tr v-for="item in inventoryRisks" :key="`${item.item}-${item.location}`" class="hover:bg-[#EBF5F5]/60">
                  ...
                  <td class="px-4 py-3 text-right font-black text-gray-900">{{ item.stock }} / {{ item.safety }}</td>
                  <td class="px-4 py-3">
                    <span class="px-2 py-1 text-[11px] font-black" :class="statusBadgeClass(item.status)">{{ item.status }}</span>
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <article class="border border-gray-200 bg-white shadow-sm">
          <div class="flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 px-4 py-3">
            <div class="flex min-w-0 items-center gap-2">
              ...
              <span class="shrink-0 text-[11px] font-black text-gray-400">{{ imbalancePositionLabel }}</span>
            </div>
            <div class="flex items-center gap-1.5">
              ...
              <button type="button" class="ml-1 text-xs font-black text-[#004D3C] hover:underline" @click="goTo('/hq/inventory/warehouse-comparison')">
                창고 비교
              </button>
            </div>
          </div>
          <div class="px-4 py-3">
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0">
                <p class="truncate font-black text-gray-900">{{ currentWarehouseImbalance.item }}</p>
                <p class="mt-1 text-[11px] font-bold text-gray-400">{{ currentWarehouseImbalance.category }}</p>
              </div>
            </div>
            ...
            <div class="grid gap-2 sm:grid-cols-2">
              ...
              >
                <div class="flex items-center justify-between gap-2">
                  <p class="truncate text-[11px] font-black text-gray-900">{{ warehouse.warehouse }}</p>
                  <span class="shrink-0 px-2 py-1 text-[10px] font-black" :class="statusBadgeClass(warehouse.status)">
                    {{ warehouse.status }}
                  </span>
                </div>
                <div class="mt-2 flex items-end justify-between gap-2">
                  <span class="text-lg font-black text-gray-950">{{ warehouse.stock }}</span>
                  <span class="text-[11px] font-bold text-gray-400">/ {{ warehouse.safety }} EA</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <article class="border border-gray-200 bg-white p-4 shadow-sm">
          <div class="flex items-center justify-between gap-3">
            ...
            <div class="flex gap-2">
              <button type="button" class="text-xs font-black text-[#004D3C] hover:underline" @click="goTo('/hq/orders')">매장 주문</button>
              <button type="button" class="text-xs font-black text-[#004D3C] hover:underline" @click="goTo('/hq/purchase-orders')">거래처 발주</button>
            </div>
          </div>
              ...
              <div class="space-y-3">
                <div v-for="status in orderStatuses" :key="status.label">
                  ...
                  <div class="mt-1 h-7 bg-gray-100">
                    <div class="h-full" :class="status.color" :style="{ width: `${Math.min(status.value, 80)}%` }" />
                  </div>
                </div>
              </div>
            </div>
        ...
        <article class="border border-gray-200 bg-white shadow-sm">
          ...
          <div class="divide-y divide-gray-100">
            <div v-for="alert in alerts" :key="alert.message" class="px-4 py-3">
              <div class="flex items-center justify-between gap-2">
                <span class="px-2 py-1 text-[11px] font-black" :class="alertTypeBadgeClass(alert.type)">{{ alert.type }}</span>
